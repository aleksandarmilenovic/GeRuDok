...
public class PasteSelected extends AbstractAction {
	public PasteSelected() {
		...
		String name = Localization.getInstance().getBundle().getString("paste");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (node instanceof SlotGM) {
			...
			if (node.equals(AppWindow.getInstance().getIzvorniSlotCopyCut())) {
		...
		// Da li je bio Cut?
		// Ako jeste obrisacemo sve elemente koji su u listi za
		// kopiranje/cutovanje kako bi sprecili vise paste-ova
		if (AppWindow.getInstance().isBioCut()) {
			// postavljamo bioCut na false kako se kod sledeceg paste ne bi
			// izvrsavao ovaj kod
			AppWindow.getInstance().setBioCut(false);
