...
public class NextProjectAction extends AbstractAction {
	...
	public NextProjectAction() {
		...
		putValue(SHORT_DESCRIPTION, Localization.getInstance().getBundle().getString("next"));
		putValue(AbstractAction.NAME, Localization.getInstance().getBundle().getString("next"));
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (tree.getLastSelectedPathComponent() instanceof ProjectM) {
			...
			try {
				if (!pmodel.getNextSibling().equals(null)) {

					TreeNode[] path = tmodel.getPathToRoot(pmodel.getNextSibling());
